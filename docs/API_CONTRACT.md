...
- **Browse time cap.** One Playwright session is capped by `REVRANK_BROWSER_TIMEOUT_SECONDS`
  ...
  `REVRANK_BROWSER_RECOVERY_ENABLED` defaults off; enablement waits on Tongli/Lead
  flag-on (not counsel clearance). Evidence `source` for browsed fields starts with
  `user_vdp_browse`. Browse runs only after Direct fail and a MarketCheck miss.
