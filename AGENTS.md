...
- `scripts/eval_imports.py` is a dry run unless given `--spend`. Narrow it first with `--only`,
  `--repeat 1` and `--source marketcheck|search|browse`.
